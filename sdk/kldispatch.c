...
int regist_net_dispatch(int type, const char* key, void* callback) {
    ...
    if (mgr->items) {
        ...
        if (hashmap_get(mgr->items, (char*)key, &netdis) == MAP_OK) {
            ...
            pnet = (rc_net_dispatch_t*)rc_malloc(sizeof(rc_net_dispatch_t) +
                                                 strlen(key));
            strcpy(pnet->key, key);
            pnet->type = type;
            pnet->callback = callback;
        ...
        LOGI("[NETDIS]", "regist type(%d), key(%s), callback(%p)", type, key,
             callback);
    }
        ...
        if (mgr->items && key != NULL) {
            ...
            if (hashmap_get(mgr->items, (char*)key, &netdis) == MAP_OK) {
                ...
                if (pnet != NULL && pnet->callback != NULL) {
                    if (pnet->type == 0) {
                        rc = ((rc_netcmd_dispatch_callback)pnet->callback)(
                            key, data);
                    } else {
                        rc = ((rc_netrpc_dispatch_callback)pnet->callback)(
                            key, data, response);
                    }
                }
            } else {
...
int rc_send_netevt(const char* dest, const char* action, cJSON* body) {
    ...
    if (dest == NULL) {  // to server
        snprintf(topic, 100, "/proton/%s/%s/evt", rc_get_app_id(),
                 rc_get_client_id());
        message = format_send_event_message(action, body);
        rcode = rc_net_publish(topic, message, strlen(message));
        free(message);
    } else if (rc_get_client_id() != NULL &&
               strcmp(dest, rc_get_client_id()) == 0) {
        snprintf(topic, 100, "/proton/%s/%s/cmd", rc_get_app_id(),
                 rc_get_client_id());
        message = format_send_event_message(action, body);
        rcode = rc_net_publish(topic, message, strlen(message));
        free(message);
...
char* format_send_event_message(const char* action, cJSON* body) {
    ...
    BEGIN_JSON_OBJECT(root)
        ...
        cJSON_AddItemToObject(JSON(root), "data",
                              body != NULL ? body : cJSON_CreateObject());
