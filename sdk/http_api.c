...
http_request rc_http_quark_request(const char* service_name, const char* path, int timeout,
...
{
    int i;
    char t[] = "ABCDEFEGHIJKLMNOPQRSTUVWXYZAbcdefeghijklmnopqrstuvwxyz0123456789";
    int n = sizeof(t) - 1;
    for (i = 0; i < 16; ++ i) {
        buf[i] = t[rand() % n];
    }
