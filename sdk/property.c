...
char* format_query_post(rc_property_manager* mgr, const char* source, cJSON** names);
...
int rc_property_value_check(int type, void* value, rc_property_t* ppty) 
{
    ...
    switch (type) {
        ...
        case RC_PROPERTY_STRING_VALUE:
            if (ppty->string_value != NULL && strcmp(ppty->string_value, value) == 0) {
                r = 0;
                break;
            }
...
{
    rc_property_manager* mgr = (rc_property_manager*)rc_malloc(sizeof(rc_property_manager));
    ...
    mgr->property_timer = rc_timer_create(env->timermgr, porperty_retry_interval, porperty_retry_interval, on_property_timer, mgr);
...
int find_change_item(any_t attrs, const char* key, any_t value)
{
    ...
    switch (ppty->type) {
        case RC_PROPERTY_INT_VALUE:
            cJSON_AddItemToObject((cJSON*) attrs, ppty->name, cJSON_CreateNumber(ppty->int_value)); 
            break;
        case RC_PROPERTY_STRING_VALUE:
            if (ppty->string_value != NULL) {
                cJSON_AddItemToObject((cJSON*) attrs, ppty->name, cJSON_CreateString(ppty->string_value)); 
            }
            break;
        case RC_PROPERTY_DOUBLE_VALUE:
            cJSON_AddItemToObject((cJSON*) attrs, ppty->name, cJSON_CreateNumber(ppty->double_value));
            break;
        case RC_PROPERTY_BOOL_VALUE:
            cJSON_AddItemToObject((cJSON*) attrs, ppty->name, cJSON_CreateNumber(ppty->bool_value));
            break;
        default:
            break;
    }
...
int clear_change_state(any_t p, const char* key, any_t value)
{
    ...
    if (ppty->status == 1) {
        ppty->status = 0;
        if (reportresult != 0 && ppty->changed == 1 && ppty->change_timestamp <= reporttime) {
            ppty->changed = 0;
        }
    }
...
int on_remote_set_property(const char* key, cJSON* JSON(data), rc_buf_t* response)
{
    any_t propertydis;
    rc_property_t* ppty = NULL;
    rc_runtime_t* env = get_env_instance();
    ...
    JSON_ARRAY_FOREACH(data, itm)
        if (JSON(itm)->string != NULL && hashmap_get(mgr->values, JSON(itm)->string, &propertydis) == MAP_OK && propertydis != NULL) {
            ppty = (rc_property_t*)propertydis;
            if (ppty->callback != NULL) {
                ...
                case RC_PROPERTY_DOUBLE_VALUE:
                    if (JSON(itm)->type != cJSON_Number) {
                        LOGI(PROPERTY_TAG, "key(%s), server push type is not match local define(int)", ppty->name);
                        continue;
                    }
                    rc_mutex_unlock(mgr->mgr_mutex);
                    ppty->callback(ppty->name, ppty->type, 
                            ppty->type == RC_PROPERTY_INT_VALUE ? (void*)&JSON(itm)->valueint : (void*)&JSON(itm)->valuedouble);
                    rc_mutex_lock(mgr->mgr_mutex);
                    break;
                case RC_PROPERTY_BOOL_VALUE:
                    if (JSON(itm)->type != cJSON_True && JSON(itm)->type != cJSON_False) {
                        LOGI(PROPERTY_TAG, "key(%s), server push type is not match local define(bool)", ppty->name);
                        continue;
                    }
                    rc_mutex_unlock(mgr->mgr_mutex);
                ...
                case RC_PROPERTY_STRING_VALUE:
                    if (JSON(itm)->type != cJSON_String) {
                        LOGI(PROPERTY_TAG, "key(%s), server push type is not match local define(string)", ppty->name);
                        continue;
                    }
                    rc_mutex_unlock(mgr->mgr_mutex);
...
int parse_attrs_get_result(rc_property_manager* mgr, cJSON* paramresult, va_list ap, cJSON* JSON(names)) 
{
    int i = 0;
    int apirc = 0;
    cJSON* json = NULL;
    ...
    BEGIN_MAPPING_JSON(paramresult, root)
        QUARK_API_RESPONSE_BEGIN(root, apirc);
            ...
            JSON_ARRAY_FOREACH(names, key)
                ...
                if (hashmap_get(mgr->values, key, &propertydis) != MAP_OK || propertydis != NULL) {
                ...
                switch (ppty->type) {
                    case RC_PROPERTY_INT_VALUE:
                        if (json->type != cJSON_Number) {
                            LOGI(PROPERTY_TAG, "key(%s), server result type is not match local define(int)", key);
                            return -1;
                        }
                    ...
                    case RC_PROPERTY_STRING_VALUE:
                        if (json->type != cJSON_String) {
                            LOGI(PROPERTY_TAG, "key(%s), server result type is not match local define(string)", key);
                            return -1;
                        }
                    ...
                    case RC_PROPERTY_DOUBLE_VALUE:
                        if (json->type != cJSON_Number) {
                            LOGI(PROPERTY_TAG, "key(%s), server result type is not match local define(double)", key);
                            return -1;
                        }
                    ...
                    case RC_PROPERTY_BOOL_VALUE:
                        if (json->type != cJSON_True && json->type != cJSON_False) {
                            LOGI(PROPERTY_TAG, "key(%s), server result type is not match local define(bool)", key);
                            return -1;
                        }
                        *(va_arg(ap, int*)) = json->valueint;
                        break;
...
char* format_query_post(rc_property_manager* mgr, const char* keys, cJSON** names)
{
    char *sendstr = NULL;
    cJSON* arr = NULL;
