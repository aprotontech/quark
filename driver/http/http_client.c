...
extern int http_request_finish(http_request request, int status_code,
                               const char* body);

struct timeval* get_tm_diff(const struct timeval* timeout,
...
int connect_to_remote(rc_http_client_t* client, struct timeval* timeout) {
    ...
    LOGI(HC_TAG, "timeout: %d.%06d", (int)timeout->tv_sec,
         (int)timeout->tv_usec);
    setsockopt(client->socket, SOL_SOCKET, SO_RCVTIMEO, timeout,
               sizeof(struct timeval));
    setsockopt(client->socket, SOL_SOCKET, SO_SNDTIMEO, timeout,
               sizeof(struct timeval));
...
#endif
    LOGI(HC_TAG, "[sock=%d] connect host(%s), port(%d)...", client->socket, ip,
         (int)ntohs(client->remote_ip.sin_port));

    ret = connect(client->socket, (struct sockaddr*)(&client->remote_ip),
...
int http_client_init_remote_info(rc_http_client_t* client, const char* domain,
    ...
    if (ipaddr != NULL) {
        ...
        if ((result = http_manager_resolve_dns(
                 client->manager, domain, &client->remote_ip.sin_addr)) != 0) {
            LOGE(HC_TAG, "resolve_dns failed %d %s", errno, strerror(errno));
            if (client->cur_request != NULL) {
                http_request_finish(client->cur_request, result,
                                    "resolve_dns failed");
...
http_client http_client_init(http_manager mgr, const char* domain,
                             const char* ipaddr, int port) {  //
    rc_http_client_t* client =
        (rc_http_client_t*)rc_malloc(sizeof(rc_http_client_t));
...
int http_client_connect(http_client _client, int schema,
...
int http_client_recv(http_client _client, char* buf, int buf_len,
                     struct timeval* timeout) {
    rc_http_client_t* client = (rc_http_client_t*)_client;
    struct timeval tv;
    ...
    if (timeout != NULL) {
        setsockopt(client->socket, SOL_SOCKET, SO_RCVTIMEO,
                   (const char*)get_tm_diff(timeout, &tv),
                   sizeof(struct timeval));
    }
    return recv(client->socket, buf, buf_len, 0);
}

int http_client_send(http_client _client, const char* buf, int buf_len,
                     struct timeval* timeout) {
    rc_http_client_t* client = (rc_http_client_t*)_client;
    struct timeval tv;
    if (client == NULL) return RC_ERROR_INVALIDATE_HTTPCLIENT;
    ...
    if (timeout != NULL) {
        setsockopt(client->socket, SOL_SOCKET, SO_SNDTIMEO,
                   (const char*)get_tm_diff(timeout, &tv),
                   sizeof(struct timeval));
    }
    return send(client->socket, buf, buf_len, 0) == buf_len
               ? 0
               : RC_ERROR_HTTP_SEND;
}
