...
#define LOG_DEBUG \
    4  // Extra information which is not necessary for normal use (values,
       // pointers, sizes, etc).
#define LOG_VERBOSE \
    5  // Bigger chunks of debugging information, or frequent messages which can
...
#define QUARK_LOG_FORMAT(format) "%s: " format
#define LOGE(tag, format, ...)                                                \
    log_write_v(LOG_ERROR, __FILE__, __LINE__, QUARK_LOG_FORMAT(format), tag, \
                ##__VA_ARGS__);
#define LOGW(tag, format, ...)                                               \
    log_write_v(LOG_WARN, __FILE__, __LINE__, QUARK_LOG_FORMAT(format), tag, \
                ##__VA_ARGS__);
#define LOGI(tag, format, ...)                                               \
    log_write_v(LOG_INFO, __FILE__, __LINE__, QUARK_LOG_FORMAT(format), tag, \
                ##__VA_ARGS__);
#define LOGD(tag, format, ...)                                                \
    log_write_v(LOG_DEBUG, __FILE__, __LINE__, QUARK_LOG_FORMAT(format), tag, \
                ##__VA_ARGS__);
#define LOGV(tag, format, ...)                                             \
    log_write_v(LOG_VERBOSE, __FILE__, __LINE__, QUARK_LOG_FORMAT(format), \
                tag, ##__VA_ARGS__);
