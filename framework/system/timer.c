...
void* timer_tick_thread(void* mgr)
{
    ...
    while (!manager->exit_thread) {
        ...
        if (manager->heap_length > 0) {
            ...
            if (tick->tv_sec < now.tv_sec || (tick->tv_sec == now.tv_sec && tick->tv_usec <= now.tv_usec)) {
                tc = manager->heap[0];
                tc->calling = 1;
                ...
                if (tc->repeat > 0) {
                    ...
                    swap_timer_node(manager->heap, tc->idx, manager->heap_length - 1);
                    -- manager->heap_length;
                    adjust_node(manager->heap, manager->heap_length, 0);
                    LOGI(TM_TAG, "timer-manager(%p) remove non-repeat timer(%p), callback(%p)", manager, tc, tc->callback);
                }
            }
        }
        ...
        if (tc != NULL) {
            tc->calling = 0;
            if (manager->heap[tc->idx] != tc) { // this timer had stoped and the position had reused, so release it
                free(tc);
            }
        }
...
{
    int ret;
    rc_timer_manager_t* mgr = (rc_timer_manager_t*)malloc(sizeof(rc_timer_manager_t));
    if (mgr == NULL) {
...
rc_timer rc_timer_create(rc_timer_manager mgr, int tick, int repeat, rc_on_time on_time, void* usr_data)
{
    ...
    if (manager->heap_length + 1 >= manager->heap_total) { // need new buffer
        heap = (rc_timer_t**)malloc(sizeof(rc_timer_t*) * manager->heap_total * 2);
        memcpy(heap, manager->heap, sizeof(rc_timer_t*) * manager->heap_total);
        memset(heap + manager->heap_total, 0, sizeof(rc_timer_t*) * manager->heap_total);
    ...
    LOGI(TM_TAG, "tick=%d, repeat=%d, callback=%p", tick, repeat, on_time);
    LOGI(TM_TAG, "new timer(%p), tick(%d), callback(%p)", tm, tm->tick, tm->callback);
    if (manager->event) {
...
int rc_timer_stop(rc_timer timer)
{
    ...
    if (manager->heap[tm->idx] == tm) {
        ...
        i = tm->idx;
        swap_timer_node(manager->heap, tm->idx, manager->heap_length - 1); // swap it with last node
        -- manager->heap_length;
