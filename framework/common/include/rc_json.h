...
#define JSON_OBJECT_EXTRACT_BOOL(root, key)                          \
    cJSON* _json_##key = cJSON_GetObjectItem(_json_##root, #key);    \
    if (_json_##key == NULL || (_json_##key->type != cJSON_True &&   \
                                _json_##key->type != cJSON_False)) { \
...
#define FREE_JSON_OBJECT(root) cJSON_Delete(_json_##root);

#define JSON_TO_STRING(root) cJSON_PrintUnformatted(_json_##root)
